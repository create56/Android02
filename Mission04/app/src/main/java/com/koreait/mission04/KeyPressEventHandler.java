...
public class KeyPressEventHandler implements TextWatcher {
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        System.out.println("입력된 문자인 바이트 수를 세는 코드");
        // 우리가 입력한키가 화면에 보이기 전에 실행되는 메서드
        try {
            // s.toString() -> 화면에 보이는 문자를 문자열로 변환
            // String.getByte() -> 문자열을 바이트로 변환 / 영문은 1바이트로, 한글은 2바이트로
            // String.getByte("인코딩 이름") -> 문자열이 "인코딩이름"으로 인코딩 되어있으니
            // 디코딩도 똑같은 방법으로 디코딩해라
            ...
            counter.setText(strCount + " /  80바이트");
